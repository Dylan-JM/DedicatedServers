...
void ADS_MatchGameMode::SetClientInputEnabled(bool bEnabled)
{
	...
	{
		ADSPlayerController* DSPlayerController = Cast<ADSPlayerController>(Iter->Get());
...
void ADS_MatchGameMode::EndMatchForPlayerStates()
{
	for (FConstPlayerControllerIterator Iter = GetWorld()->GetPlayerControllerIterator(); Iter; ++Iter)
	{
		if (ADSPlayerController* DSPlayerController = Cast<ADSPlayerController>(Iter->Get()))
		{
			if (ADS_MatchPlayerState* MatchPlayerState = DSPlayerController->GetPlayerState<ADS_MatchPlayerState>())
			{
