...
void USignUpPage::UpdateSignUpButtonState(const FText& Text)
{
	...
	const bool bIsUsernameValid = !TextBox_Password->GetText().ToString().IsEmpty();
	const bool bArePasswordsEqual = TextBox_Password->GetText().ToString().Equals(TextBox_ConfirmPassword->GetText().ToString());
...
bool USignUpPage::IsStrongPassword(const FString& Password, FString& StatusMessage)
{
	...
	{
		StatusMessage = TEXT("Passwords must be at least 8 characters.");
		return false;
	}
	return true;
}
